...
Adapted from the `skills/caveman` style-mode skill in [JuliusBrussee/caveman](https://github.com/JuliusBrussee/caveman) (MIT license; see `LICENSE.txt`). Only this base style skill was vendored here — the source project's compression engine, proxy, MCP server, `cavecrew` subagents, and `caveman-compress`/`caveman-stats` companion skills are BSL-licensed or depend on hooks and infrastructure outside the scope of this fork; install the full project separately if you want those.