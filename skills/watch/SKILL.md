...
2. Run `bash watch.sh <args>` from the skill directory. It will:
...
`yt-dlp` and `ffmpeg` must already be on `PATH` (`brew install yt-dlp ffmpeg`, or your package manager). Transcription needs no key for videos with native captions; otherwise set `ELEVENLABS_API_KEY` or `GROQ_API_KEY` in the environment or a `.env` file next to `watch.sh`.

## Source

Adapted from [alexlarcheveque/claude-watch](https://github.com/alexlarcheveque/claude-watch) (MIT license; see `LICENSE.txt`).
